...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn mint(
			...
			let signed_origin = ensure_signed(origin)?;
			// - Ensure the caller is the asset owner.
			Self::ensure_is_owner(asset_id, signed_origin)?;
		...
		pub fn burn(origin: OriginFor<T>, asset_id: AssetId, amount: u128) -> DispatchResult {
			...
			Asset::<T>::try_mutate(asset_id, |maybe_details| -> DispatchResult {
				...
				Account::<T>::mutate(asset_id, signed_origin.clone(), |balance| {
		...
		pub fn transfer(
			...
			// - Ensure the asset actually exists
			ensure!(Self::asset(asset_id).is_some(), Error::<T>::UnknownAssetId);

			// - Mutate both account balances.
			// Sub from first account + update balance
			let mut xfered_amount = 0;
			Account::<T>::mutate(asset_id, signed_origin.clone(), |balance|{
				let old_balance = *balance;
				*balance = balance.saturating_sub(amount);
				xfered_amount = old_balance - *balance;
			});
			// Add xfered_amount to bal of dest account
			Account::<T>::mutate(asset_id, to.clone(), |balance| {
				*balance = balance.saturating_add(xfered_amount);
			});
